...
function Examples({ chainId }: ExampleProps) {
	...
	const getStakingPool = async () => {
		try {
			const queryClient = await StargateClient.connect(defaultUrls[chainId].rpc);
			// TODO: Implement staking pool query with new CosmJS
			// For now, set mock data
			setPool({ bonded_tokens: 1000000, not_bonded_tokens: 500000 });
			console.log('Staking pool query connected successfully', queryClient);
		} catch (error) {
			console.error('Failed to connect to staking pool:', error);
		}
	};
	...
	const fetchCount = async () => {
		const cosmWasmClient = await CosmWasmClient.connect(defaultUrls[chainId].rpc);
	...
	const incrementCounter = async () => {
		...
		const signingCosmWasmClient = await SigningCosmWasmClient.connectWithSigner(defaultUrls[chainId].rpc, offlineSigner);
